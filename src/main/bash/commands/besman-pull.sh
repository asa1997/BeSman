...
function __besman_fetch_steps_file() {
    ...
    local steps_file_base_name="besman-$playbook_name-playbook-$playbook_version-steps"
    local download_url
    # to get the extension of the file
    download_url=$(curl -s "$API" | jq -r --arg name "$steps_file_base_name" '.[] | select(.name | test("^" + $name + "\\.(sh|md|ipynb)$")) | .download_url')

    if [[ -n "$download_url" ]]; then
        __besman_echo_white "Downloading steps file from $download_url"
        curl -L "$download_url" -o "${download_url##*/}"
    else
        echo "No matching file found in GitHub API."
        exit 1
    fi
}